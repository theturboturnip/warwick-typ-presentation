...
\begin{wideitemize}
    \item This program is an example of `tightly-coupled in-situ visualization'.\todocite{https://www.cs.uoregon.edu/Reports/AREA-201703-Kress.pdf}
    \item Academia hasn't recently innovated in fluid visualization, only in methods for running faster such as \todocite{https://ieeexplore.ieee.org/document/506222}.
    \item This was noted in \todocite{https://ieeexplore.ieee.org/document/1297005}, which states `feature detection' would be a key element going forward rather than new visualization methods.
\end{wideitemize}
\end{frame}
...
\begin{wideitemize}
    ...
    \item The MET office also uses conventional visualization methods in weather reports, but combines and filters them to show a wide range of information.
\end{wideitemize}
}{
    \begin{figure}
        \centering
        \includegraphics[width=0.7\textwidth]{Presentation/images/weather.PNG}
        \caption{Weather Forecast showing wind speed, weather fronts, and cloud cover. \todocite{https://www.youtube.com/watch?v=y_1--MkiNjQ, Met Office 10 Day Trend for March 3rd.}}
    \end{figure}
}
...
\begin{frame}[fragile]{Viz Compute - Extracting Simulation Data}
...
\begin{frame}{GPU Synchronization I}
% Semaphores
    \makebox[\textwidth][c]{
    \begin{tikzpicture}[
            scale=0.9, every node/.style={scale=0.9},
    ...
    \begin{wideitemize}
        ...
        \item One workload waits on a semaphore until another workload signals it\todocite{one workload per semaphore}.
...
\begin{frame}{Per-Layer Viz Work}
    \makebox[\textwidth][c]{
        \begin{tikzpicture}[
            ...
            \node[fixedrect](sq_r) at (9.5, 0){Draw\\Background w/ Quantity};
...
\begin{frame}{Vulkan Commands}
% Perform each phase in series
    \makebox[\textwidth][c]{
        \begin{tikzpicture}[
            scale=0.9, every node/.style={scale=0.9},
            fixedrect/.style={rectangle,draw,minimum height=4em,anchor=west,text width=2cm,align=center},
            ...
            \node[fixedrect](sq1) at (prelim.east){Extract Scalar Quantity};
            \node[fixedrect](sq2) at (sq1.east){Find Scalar min/max};
            \node[fixedrect](vq1) at (sq2.east){Extract Vector Quantity};
            \node[fixedrect](vq2) at (vq1.east){Find Vector min/max};
            \node[fixedrect](vq3) at (vq2.east){Create Vector Instances};
            ...
            \basevizvulkancommand{}
        \end{tikzpicture}
    }
    % \vfill\null
    \begin{wideitemize}
        \item The compute work for each layer is recorded one after the other on the same command buffer.
        \item The layer Compute work cannot run in parallel, which could be improved in the future.
        \item Graphics work cannot run in parallel, because they could write to the same pixels.
    \end{wideitemize}
\end{frame}

\begin{frame}{Vulkan Commands - Ordering}
% Recording them in order doesn't guarantee they execute in order
% If you execute B, no guarantee you'll see the results of A (i.e. memory writes).
\end{frame}

\begin{frame}{Vulkan Commands - Memory Barriers}
% Add memory barriers to ensure memory accesses
% accesses of T1 from stage S1 guaranteed visible to accesses of T2 from S2
\end{frame}
